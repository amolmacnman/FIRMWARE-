...
<code>resp</code>. <code>set_threshold</code> with <code>param:"cadence"</code> maps to
<code>SW_DN_SET_CADENCE</code> and retunes a node's advertising period over the air.</p>
<div class="warn"><b>&sect;5.1.10 thermal transmit halt.</b> Before any uplink the gateway reads
the die temperature and <b>refuses to transmit above 85&nbsp;&deg;C</b>, resuming below
75&nbsp;&deg;C. The 10&nbsp;&deg;C hysteresis stops it oscillating at the threshold. It <b>fails
open</b>: if the sensor is unavailable the gateway transmits rather than going silent, because
a dead thermometer must not look like a dead wagon.</div>
<table>
...
<tr><td>Gateway &rarr; sub-node</td><td>BLE GATT, during the node's config window</td><td>thresholds, <b>and firmware</b> - sealed 192-byte chunks staged in the NOR <code>ota</code> partition</td></tr>
</table>
<h3>5.1 Sub-node firmware update</h3>
<p>A node is updated by <b>its own gateway only</b> - there is no SMP/MCUmgr service on any node and no
phone path. <code>ota_start</code> carrying a <code>node</code> field stages the image over HTTP into
the NOR <code>ota</code> partition (512 KB), then <code>node_link</code> streams it as sealed 192-byte
chunks whenever that node opens a window.</p>
<table>
<tr><th>Property</th><th>Behaviour</th></tr>
<tr><td>Scope</td><td><b>One node per command.</b> NODE_ID is compiled into the node binary, so nodes never share an image. "The same node on every wagon" is done by broadcasting the command to <code>smartwagon/v1/all/dn/cmd</code> instead.</td></tr>
<tr><td>Staging</td><td>Required, not optional: the modem and a node window are never up together, so the image cannot be piped from HTTP to BLE.</td></tr>
<tr><td>Resume</td><td>Campaign target, size, CRC and bytes-delivered live in FRAM. A gateway reset resumes at the last chunk the NODE acknowledged, never at what we hoped it took.</td></tr>
<tr><td>Ordering</td><td>The node accepts chunks strictly in order, so a partial window can never leave a hole in the image.</td></tr>
<tr><td>Nonces</td><td>Direction tag <code>0x02</code> for image chunks vs <code>0x01</code> config and <code>0x00</code> adverts, with a persisted counter reserved 1024 at a time - a reset skips values, never reuses one.</td></tr>
<tr><td>Speed</td><td><code>SW_DN_FASTWIN</code> is queued with the campaign so windows open every few seconds instead of every 10 minutes. Fast mode is deadlined in the node, so an abandoned campaign cannot strand it at high duty.</td></tr>
<tr><td>Acceptance</td><td>CRC16 over the whole image, then MCUboot signature verification with auto-revert. The node swaps on its own next reset, so an update never interrupts a report.</td></tr>
<tr><td>Confirmation</td><td>Both the gateway and the node run MCUboot in <code>BOOT_UPGRADE_TEST</code> and call <code>boot_write_img_confirmed()</code> only after one complete successful cycle. Until that call the previous image is still the fallback, so a build that boots but cannot reach the broker - or cannot read its sensor - is reverted automatically rather than stranding the unit.</td></tr>
</table>
...
<tr><td>Offline buffer</td><td>NOR ring to the end of the chip (&sect;7.5 needs 1 month)</td></tr>
<tr><td>Sub-node image staging</td><td>512 KB NOR <code>ota</code> partition, one campaign at a time</td></tr>
...
<tr><td>EC200U modem (SW_GNSS, P2.08)</td><td>gated</td><td>only to publish; AT+QPOWD then off - always, every cycle. The MODEM_HOLD_SESSION bench switch that kept the session attached has been removed.</td></tr>
...
<div class="warn"><b>The gateway runs on the custom board and publishes to the MQTT broker.</b>
GNSS, the EC200U modem, the NOR flash, the SHT40, the BMA400 and the BQ25798 are alive on hardware.
The pin map below was verified against the gateway schematic netlist; the items still marked open
are firmware gaps or bench checks to close before production flashing.</div>
<div class="warn"><b>The modem was silent because <code>app.overlay</code> reversed the UART.</b>
It redefined <code>uart22_default</code> with TX and RX swapped, overriding the corrected
<code>sw_gateway-pinctrl.dtsi</code>. <code>all_hw_test</code> defines no <code>uart22</code>
pinctrl of its own, so it picked the board fix up and talked to the modem immediately - which is
precisely why the test worked and production did not. Per the netlist, EC200U <code>MAIN_TXD</code>
(module pin 67, an output) reaches nRF pin 9 = <b>P1.07</b>, so P1.07 must be <i>our</i> RX. Earlier
revisions of this sheet named <code>GNSS_3V3</code> as the prime suspect; it was not.</div>
<table>
...
<tr><td><code>uart22</code> TX/RX vs. EC200U through the TXS0102</td><td><b>Verified on hardware</b> - our TX = P1.06, our RX = P1.07. Fixed in both <code>sw_gateway-pinctrl.dtsi</code> and <code>app.overlay</code>; the overlay was the one that mattered.</td></tr>
<tr><td><code>GNSS_3V3</code> (P2.09) &rarr; TXS0102 pin 7 (VCCB)</td><td><b>Closed</b> - raised by <code>modem_power_on()</code> as part of the comms-rail sequence. It was <i>not</i> the cause of the silent modem.</td></tr>
...
<tr><td>PWRKEY / RESET polarity through T1/T2</td><td><b>Verified on hardware</b> - GPIO high pulls the modem line low. PWRKEY is a <b>toggle</b>, not an on-switch, so it is pulsed only after AT has been tried and got no answer; pulsing a running module powers it down.</td></tr>
<tr><td>FRAM (IC3)</td><td><b>OPEN - not fitted.</b> The gateway logs <code>config: seeded defaults</code> every boot, so no setting survives a power cycle and RDSO &sect;7.5 is not truly met.</td></tr>
<tr><td>BQ25798 <code>VREG</code> / cell count</td><td><b>OPEN</b> - firmware writes only <code>ADC_CTRL</code> (0x2E); charge voltage and cell count are left at the power-on defaults set by the PROG-pin strap. Confirm that strap before leaving a real 2S LTO pack charging unattended.</td></tr>
</table>
{REF}
'''
...
<li>Advertise the encrypted burst (non-connectable); <b>slow when normal (~30 s), fast while in alarm (~1 s)</b>. Both periods are read at runtime from <code>sn_normal_s()</code> / <code>sn_fast_s()</code>, so a <code>SW_DN_SET_CADENCE</code> downlink changes them live and the change survives a reboot (section 5.1). A <code>DEBUG_TRACE</code> build seeds <b>10 s</b> instead of 30 s so a node is visible on the bench without waiting half a minute per advert - the seed only, not a different mechanism.</li>
<li>Every CFGWIN_PERIOD_MS of wall-clock time - <b>including during an alarm</b> - open a 4 s
connectable <b>config window</b> so the gateway can write new thresholds, or stream a firmware image
(section 6).</li>
</ol>
...
<h2>3.1 Cadence over the air (<code>SW_DN_SET_CADENCE</code>, 0x08)</h2>
<p>RDSO &sect;7.15 requires the vendor to set operating values in the field, so the advertising
period is not fixed at build time. The command carries the <b>quiet</b> period in
<code>value</code> and the <b>alarm</b> period in <code>hyst</code>, both in <b>seconds</b> - the
same two wire fields every other threshold uses, so it inherits the individual / by-type /
all-nodes fan-out unchanged.</p>
<table>
<tr><th>Guard</th><th>Behaviour</th></tr>
<tr><td>Range</td><td>Both clamped to <code>CADENCE_MIN_S</code> 1 s &hellip; <code>CADENCE_MAX_S</code> 3600 s. Below 1 s buys nothing over the alarm cadence and costs battery; above an hour the node cannot satisfy the gateway's 15-minute staleness check and would be flagged DOWN continuously.</td></tr>
<tr><td>Ordering</td><td>The alarm period is never allowed to exceed the quiet one. An alarm reporting <i>less</i> often than normal traffic would invert the purpose of the fast cadence.</td></tr>
<tr><td>Persistence</td><td>Written to NVS (<code>sw/norm</code>, <code>sw/fast</code>) and reloaded by <code>settings_load()</code>, so a cadence tuned over the air survives both a reboot <b>and</b> a firmware update carrying different compile-time defaults.</td></tr>
<tr><td>Effect</td><td>Immediate - the very next sleep uses the new value. No reboot.</td></tr>
</table>
<div class="warn"><b>Battery consequence:</b> the &sect;5.1.20 six-year projection is built on the
<b>30 s</b> production figure. Dropping the quiet cadence to 5 s multiplies advertising charge by
six and invalidates the life estimate - the clamp bounds the damage, it does not prevent it.</div>
...
<h2>6. Firmware update over BLE</h2>
<p>The node can be updated by <b>its own gateway only</b>. There is no SMP service, no MCUmgr, and no
phone path - the image arrives as sealed chunks on a second GATT characteristic during the same
config window used for thresholds.</p>
<table>
<tr><th>Step</th><th>What happens</th></tr>
<tr><td>Stage</td><td>Gateway downloads the image over HTTP into its NOR <code>ota</code> partition and computes a CRC16. The two links are never up together, so the image must be staged before any of it can be sent.</td></tr>
<tr><td>Arm</td><td><code>SW_DN_OTA_BEGIN</code> on the config characteristic: erases this node's MCUboot secondary slot and announces the size. Sent <b>once</b> per campaign - a resumed transfer must not repeat it.</td></tr>
<tr><td>Stream</td><td>192-byte chunks, each its own AES-CCM frame with nonce direction tag <code>0x02</code>, accepted only <b>strictly in order</b>. Delivery spans as many windows as it takes; progress is persisted in gateway FRAM so a reset resumes rather than restarts.</td></tr>
<tr><td>Verify</td><td><code>SW_DN_OTA_END</code> carries the CRC16. A mismatch discards the whole transfer before anything is marked bootable.</td></tr>
<tr><td>Swap</td><td><code>boot_request_upgrade(BOOT_UPGRADE_TEST)</code>. <b>No reboot here</b> - the node swaps on its own next reset, so an update never interrupts a report in flight. MCUboot verifies the fleet signature and <b>auto-reverts</b> if the new image fails to confirm.</td></tr>
<tr><td>Confirm</td><td><code>boot_write_img_confirmed()</code>, called only after the new image completes a <b>full successful cycle</b> - sense, seal, advertise, account. <b>TEST mode without this step reverts on the second reboot</b>, so an update that appeared to install would silently vanish days later. Deferring the confirm until a whole cycle has passed is what makes the auto-revert meaningful: an image that boots but cannot read its sensor is rejected by the node itself.</td></tr>
</table>
<p class="small"><b>Three independent gates</b>, in order: per-chunk AES-CCM (a neighbouring wagon or an
attacker cannot inject one byte), CRC16 over the whole image (catches truncation or reordering before
anything is bootable), and MCUboot signature verification with auto-revert (the final authority).
Gate 3 alone would stop unsigned code running, but not stop a wagon spending hours of radio assembling
rubbish - that is what 1 and 2 buy.</p>
<p class="small">During a campaign the gateway sends <code>SW_DN_FASTWIN</code> so windows open every
few seconds instead of every 10 minutes. Fast mode is <b>deadlined</b> in the node, so an abandoned
campaign or a lost cancel cannot strand it at high duty.</p>
{extra_html}
{REF}
'''
...
subnode("Subnode_Bearing", "Sub-node — Bearing &amp; Wheel Health",
        ...
        "temp &ge; sn_threshold()?\\n(this node does NOT decide flat-wheel)",
        ...
        [("Bearing over-temp / seizure", "HOT_BEARING", "node decides; sev red"),
         ("Wheel flat / RCF (vibration)", "FLAT_WHEEL",
          "GATEWAY decides - needs speed &gt; 15 km/h (&sect;7.9)"),
         ("Cell low", "NODE_LOW_BATTERY", "advisory, rides next heartbeat")],
        extra_html='''<h2>6. Vibration: a hint from the node, a decision by the gateway</h2>
<p>The node measures the vibration index into <code>value2</code> but <b>does not raise
FLAT_WHEEL</b>. That test is only valid above 15 km/h, and a bearing node has no
accelerometer and no speed input - it cannot tell rolling from shunting, so letting it
alarm autonomously would fire during yard handling.</p>
<p>What it does instead: crossing its own <code>sn_vib_threshold()</code> sets an internal
<code>VIB_HIGH</code> flag and drops the advert period from ~30 s to <b>~1 s</b>, so the
gateway - which knows the speed - gets fresh data within about a second. Before this the
node escalated for its own over-temperature alarm but not for a gateway-decided one, so a
flat wheel could be reported up to 30&times; slower than an over-temperature on the same
node. RDSO &sect;7.3 asks for a "near real time alert" once a threshold is exceeded.</p>
<div class="note">Keep the node hint limit <b>at or below</b> the gateway's
<code>FLAT_WHEEL_VIB_THRESH</code>. A node limit above it leaves a band where the gateway
would alarm but the node never speeds up, putting the latency straight back.
Settable per node: <code>set_threshold</code> with <code>"param":"vib_index"</code>.</div>

<h2>7. What is missing</h2>
<ul>
